...
export const ThemeSwitcher = ({ className }: { className?: string }) => {
  ...
  if (!mounted) {
    return <div className={cn("relative isolate flex h-8 w-[104px] rounded-full bg-background p-1 ring-1 ring-border", className)} />;
  }
  ...
  return (
    <div className={cn("relative isolate flex h-8 rounded-full bg-background p-1 ring-1 ring-border", className)}>
      {themes.map(({ key, icon: Icon, label }) => {
        ...
        return (
          <button aria-label={label} className="relative h-6 w-6 rounded-full" key={key} onClick={() => setTheme(key)} type="button">
            {isActive && (
